...
import { CreateTourData, createTour } from '@/app/lib/services/tourService';
...
const createTourSchema = (dict: DictType) => {
...
export async function createTourAction(
  data: CreateTourData & { locale?: Locale }
): Promise<{
  success: boolean;
  ...
  try {
    ...
    const validationResult = createTourSchema(dict).safeParse(data);
